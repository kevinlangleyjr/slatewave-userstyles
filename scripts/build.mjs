#!/usr/bin/env node
// Build slatewave.user.css for each style in styles/.
//
// Reads styles/<site>/meta.json + styles/<site>/style.css and emits a
// Stylus-compatible UserCSS file at styles/<site>/slatewave.user.css.
//
// Pass --check to fail (exit 1) if any built artifact would change. Useful in CI
// to ensure committed .user.css files are in sync with their source.
...
async function main() {
  ...
  if (checkOnly) {
    if (drift.length > 0) {
      console.error(`out-of-date: ${drift.map((r) => r.site).join(", ")}`);
      console.error(`run \`npm run build\` and commit the changes.`);
      process.exitCode = 1;
    } else {
      console.log(`all ${ok.length} styles up to date`);
    }
    return;
  }
